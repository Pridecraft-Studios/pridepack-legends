...
const _CreateBaseAi = (villageId, AiData, AiName, rallyData = [], overrideSpawnLocation = EMPTY_ENTITY_GROUP) => {
    ...
    if (rallyData.length > 0) {
        rallyData.forEach((compositionName) => {
            const controlGroup = CreateControlGroupEntity(villageId, spawnEntity, compositionName)
            OUTPUT_ControlGroupFollowAI(controlGroup, newAi)
        })
    }
...
SNIPPET_BuildableSpawnerBatchSpawned("bss_control_group_recruit_spawned", (buildableSpawner, spawnedEntities, payload) => {
    ...
    if (payload.string !== "") {
        ...
        AssignEntitiesToAI(villageId, spawnedEntities, aiConfigController)
        ...
        if (HasEntities(ControlGroupEntity)) {
            ...
            OUTPUT_MoveAttackEntity(spawnedEntities, ControlGroupEntity, CALLBACK_NONE)
        }
    }
})
//==================================Two rally points==================================
...
SNIPPET_BuildableSpawnerBatchSpawned("bss_control_group_recruit_spawned_barracksdefendHordeSiege0_barracksdefendHordeDefend0", (buildableSpawner, spawnedEntities, _payload) => {
    const villageId = QUERY_GetVillageIDFromEntity(buildableSpawner)
    const aiConfig = GetAiConfigOfBase(villageId)
    const halfSpawnedEntities0 = FILTER_RandomPercentage(spawnedEntities, 50)
    const halfSpawnedEntities1 = OPER_Difference(spawnedEntities, halfSpawnedEntities0)
    ...
    if (HasEntities(rallyPoint0)) {
        OUTPUT_MoveAttackEntity(halfSpawnedEntities0, rallyPoint0, CALLBACK_NONE)
    }
    if (HasEntities(rallyPoint1)) {
        OUTPUT_MoveAttackEntity(halfSpawnedEntities1, rallyPoint1, CALLBACK_NONE)
    }
})
//=======Unique-Defend==========
SNIPPET_BuildableSpawnerBatchSpawned("bss_control_group_recruit_spawned_barracksdefendHordeUnique0_barracksdefendHordeDefend0", (buildableSpawner, spawnedEntities, _payload) => {
    const villageId = QUERY_GetVillageIDFromEntity(buildableSpawner)
    const aiConfig = GetAiConfigOfBase(villageId)
    const halfSpawnedEntities0 = FILTER_RandomPercentage(spawnedEntities, 50)
    const halfSpawnedEntities1 = OPER_Difference(spawnedEntities, halfSpawnedEntities0)

    const aiConfigController0 = aiConfig[DEFEND_UNIQUE_AI.TAG]
    AssignEntitiesToAI(villageId, halfSpawnedEntities0, aiConfigController0)
    const aiConfigController1 = aiConfig[DEFEND_DEFEND_AI.TAG]
    AssignEntitiesToAI(villageId, halfSpawnedEntities1, aiConfigController1)

    const rallyPointData0 = GetDefendRallyPoints(villageId)[DEFEND_UNIQUE_AI.CG_0]
    const rallyPointData1 = GetDefendRallyPoints(villageId)[DEFEND_DEFEND_AI.CG_0]
    const rallyPoint0 = GetNearestCompositionRallypoint(villageId, buildableSpawner, rallyPointData0)
    const rallyPoint1 = GetNearestCompositionRallypoint(villageId, buildableSpawner, rallyPointData1)
...
//=======Siege-Unique==========
SNIPPET_BuildableSpawnerBatchSpawned("bss_control_group_recruit_spawned_barracksdefendHordeSiege0_barracksdefendHordeUnique0", (buildableSpawner, spawnedEntities, _payload) => {
    const villageId = QUERY_GetVillageIDFromEntity(buildableSpawner)
    const aiConfig = GetAiConfigOfBase(villageId)
    const halfSpawnedEntities0 = FILTER_RandomPercentage(spawnedEntities, 50)
    const halfSpawnedEntities1 = OPER_Difference(spawnedEntities, halfSpawnedEntities0)
    ...
    const aiConfigController0 = aiConfig[DEFEND_SIEGE_AI.TAG]
    AssignEntitiesToAI(villageId, halfSpawnedEntities0, aiConfigController0)
    const aiConfigController1 = aiConfig[DEFEND_UNIQUE_AI.TAG]
    AssignEntitiesToAI(villageId, halfSpawnedEntities1, aiConfigController1)

    const rallyPointData0 = GetDefendRallyPoints(villageId)[DEFEND_SIEGE_AI.CG_0]
    const rallyPointData1 = GetDefendRallyPoints(villageId)[DEFEND_UNIQUE_AI.CG_0]
    const rallyPoint0 = GetNearestCompositionRallypoint(villageId, buildableSpawner, rallyPointData0)
    const rallyPoint1 = GetNearestCompositionRallypoint(villageId, buildableSpawner, rallyPointData1)
...
SNIPPET_BuildableSpawnerBatchSpawned("bss_control_group_recruit_spawned_final", (buildableSpawner, spawnedEntities, payload) => {
    ...
    if (bastionString !== "") {
        ...
        if (HasEntities(relativeRallyPoint)) {
            OUTPUT_MoveAttackEntity(spawnedEntities, relativeRallyPoint, CALLBACK_NONE)
        } else {
            const boss = QUERY_GetEntitiesWithTags("finalBoss")
            if (HasEntities(boss)) {
                if (HasEntities(spawnedEntities)) {
                    OUTPUT_MoveAttackEntity(spawnedEntities, boss, CALLBACK_NONE)
                }
            }
        }
    } else {
...
SNIPPET_LocalTimer("lt_move_patrol_units", (payload) => {
    ...
    if (HasEntities(aroundRallyPoint0)) {
        OUTPUT_MoveAttackEntity(aroundRallyPoint0, patrolRallyPoint1, CALLBACK_NONE)
    }
    if (HasEntities(aroundRallyPoint1)) {
        OUTPUT_MoveAttackEntity(aroundRallyPoint1, patrolRallyPoint0, CALLBACK_NONE)
    }
                ...
                if (nonStationedUnitsCount >= 6) {
                    ...
                    if (HasEntities(unitsToSend)) {
                        ...
                        if (HasEntities(otherRallyPoints)) {
                            OUTPUT_MoveAttackEntity(unitsToSend, FILTER_ByClosest(otherRallyPoints, singleIntruder, 1), CALLBACK_NONE)
        ...
        if (HasEntities(deBuildable)) {
            ForEachEntities(deBuildable, (buildingEntity) => {
                ...
                if (HasEntities(piglins)) {
                    OUTPUT_SetLeash(piglins, buildingEntity, deData.type.leashDistance, deData.type.returnDistance)
                }
            })
        }
    }
...
SNIPPET_BuildingComplete("bc_attack_discrete_encounter", (buildingEntity, payload) => {
    ...
    if (IncrementGlobal(villageId + "_" + deKey) >= deData.amount) {
        Once()
    }
})
