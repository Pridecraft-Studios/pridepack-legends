...
const _TryAddRewardsForAct1 = (villageId) => {
    const store = VillageResourceStore(villageId)
    const newWoodAmount = 1000
    const newStoneAmount = 1000
    const newLapisAmount = 150
...
const _MessageCollectedRewards = (collectedRewards, messageData) => {
    ...
    if (!hasCollected) {
        if (hasExcess) {
            OUTPUT_Announce(messageData.cannotOpenNoSpace, [])
        } else {
            OUTPUT_Announce(messageData.cannotOpenEmpty, [])
        }
        return
    }
    ...
    if (hasExcess) {
        OUTPUT_Announce(messageData.openExcess, [rewardsContentString])
    } else {
        OUTPUT_Announce(messageData.open, [rewardsContentString])
    }
}
...
const CollectRewardsFromAllVillages = (playerEntity) => {
    ...
    _MessageCollectedRewards(collectedRewards, villageRewardData.messages.all)
...
const CollectRewardsFromVillage = (playerEntity, villageId) => {
    ...
    _MessageCollectedRewards(collectedRewards, villageRewardData.messages.single)
...
const _HandleAct1VillageChest = (playerEntity, interactedEntity, villageId) => {
    ...
    if (QUERY_GetGlobalVariable("gv_onboarding_village_chest_active") > 0) {
        ...
        if (QUERY_HasTags(interactedEntity, ["secondVillageChest"])) {
            ...
            OUTPUT_RemoveTag(interactedEntity, "secondVillageChest")
            FinishVillageChestStep() //In act1_flow
        } else {
            OUTPUT_Announce(villageRewardData.messages.single.cannotOpenEmpty, [])
        }
...
SNIPPET_PlayerInteracted("pi_village_chest_interacted_campaign", (playerEntity, interactedEntity, _payload) => {
    ...
    if (QUERY_IsVillageOccupied(villageId)) {
        OUTPUT_Announce("village_chest_cannot_open_piglin_occupied", [])
        return
    }
