...
SNIPPET_EntityDisabled("ee_fountain_disabled", (fountainEntity) => {
    ...
    const villageEntity = GetVillageEntityFromID(QUERY_GetVillageIDFromEntity(fountainEntity))
    if (QUERY_HasTags(villageEntity, ["secondVillageUnderAttack"]) || QUERY_HasTags(villageEntity, ["firstVillageUnderAttack"])) {
        return
    }
...
SNIPPET_SpatialPartitionEntered("spe_village_cine_and_vo", (triggerEntity, _intruderEntity, _payload) => {
    ...
    if (QUERY_HasTags(villageEntity, ["secondVillageUnderAttack"]) || QUERY_HasTags(villageEntity, ["firstVillageUnderAttack"]) || QUERY_HasTags(villageEntity, ["thirdVillageUnderAttack"])) {
        return
    }
