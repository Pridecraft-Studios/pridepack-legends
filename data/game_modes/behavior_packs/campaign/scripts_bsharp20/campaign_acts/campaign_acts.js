...
const _DBBPlacementTiers = () => {
    const placementTiers = [
        { playersMin: 500, sKMin: 450, sKMax: 1000, wofMin: 650, friendlyVillagesMin: 350, poiMin: 300, maMin: 300 },
        { playersMin: 500, sKMin: 450, sKMax: 1000, wofMin: 650, friendlyVillagesMin: 350, poiMin: 350, maMin: 350 },
        { playersMin: 500, sKMin: 500, sKMax: 1500, wofMin: 700, friendlyVillagesMin: 350, poiMin: 400, maMin: 400 },
        { playersMin: 500, sKMin: 550, sKMax: 1500, wofMin: 750, friendlyVillagesMin: 350, poiMin: 450, maMin: 450 },
        { playersMin: 500, sKMin: 550, sKMax: 2000, wofMin: 750, friendlyVillagesMin: 350, poiMin: 500, maMin: 500 },
        { playersMin: 500, sKMin: 550, sKMax: 2000, wofMin: 750, friendlyVillagesMin: 350, poiMin: 550, maMin: 550 }
    ]
    ...
    const allPiglinOutposts = FILTER_ByFactionName(QUERY_GetAllVillages(), FACTION_NAME_DBB)
    const sK = FILTER_ByVillageSize(allPiglinOutposts, BASE_SIZE_ACT1_NON_PORTAL_ATTACK_BASE)
    const primaryRef = HasEntities(sK) ? sK : RandomEntity(allPiglinOutposts)
    ...
    if (HasEntities(primaryRef)) {
        for (let i = 0; i < placementTiers.length; i++) {
            ...
            OUTPUT_PlacementSetPrimaryProximityRule(primaryRef, placement.sKMin, placement.sKMax)
...
SNIPPET_CinematicEvent("ce_dbb_generate_act2_bases", () => {
    ...
    const hordesFinishedBuildingOutposts = {}
    for (const piglinHorde of FACTION_PIGLINS) {
        hordesFinishedBuildingOutposts[piglinHorde] = false
    }

    while (buildingOutposts) {
        const horde = FACTION_PIGLINS[hordeIndex % FACTION_PIGLINS.length] // will always be 0, 1, 2

        const totalOutpostsSoFar = QUERY_GetEntitiesCount(FILTER_ByVillageSize(FILTER_ByFactionName(QUERY_GetAllAliveVillages(), horde), BASE_SIZE_OUTPOST))
        if (totalOutpostsSoFar < HordeBasesSetup[horde].numInitialOutposts) {
            PlacePiglinOutpost(horde)
        } else {
            hordesFinishedBuildingOutposts[horde] = true
        }

        // Check if hordes are done
        buildingOutposts = false
        for (const hordeToCheck of FACTION_PIGLINS) {
            if (!hordesFinishedBuildingOutposts[hordeToCheck]) {
