...
SNIPPET_NonPopCappedEntityDestroyed("ed_poi_tower_deconstructed", (destroyedEntity, payload) => {
    const towerData = poiTowerVal[payload.string]
    const villageEntity = GetVillageEntityFromID(QUERY_GetVillageIDFromEntity(destroyedEntity))
    OUTPUT_MapSetKeyValue(villageEntity, MAP_KEY.VISIBILITY, MAP_VISIBILITY.HIDDEN)
    SetEntityWaypointIconState(villageEntity, towerData.id, POI_WAYPOINT_ICON_STATE.HIDDEN)
    ...
    if (!IsCurrentAct(ACTS.EPILOGUE) && !IsCurrentAct(ACTS.ACT_3A) && DoOnce("gv_deconstruct_once_act2")) {
        PlayPresentationActionToClosePlayers(towersMisc.deconstructAct2FirstTimeVO, villageEntity)
    } else {
...
const _SetPOITowersInteractionState = (towerKey, state) => {
    const allTowers = FILTER_ByFactionName(QUERY_GetEntitiesWithTags(poiTowerVal[towerKey].buildingTag), [POI_TOWER_NAME_KNOCKBACK, POI_TOWER_NAME_STUN, POI_TOWER_NAME_FREEZE])
