...
const _SendPiglinBaseDestroyedMessage = (villageId) => {
    ...
    if (!QUERY_GetGlobalVariable(BOSS_BASE_VALS.globals.factionMeetTheBossPlayed + factionName)) {
        //message = "invasion_outpost_base_destroyed_unknown_sequence" //Disabling for now, as we're removing the pre-game and thus the need to obscure horde name.
    }

    const collectedRewards = CollectDynamicBaseLoot(villageId)
    // note: this should be using purely numbers and images to be localization safe.
    let rewardsContentString = ""
    for (const resourceName in collectedRewards) {
        rewardsContentString += `${collectedRewards[resourceName]}|+[key.item.${resourceName}]| `
        Logv(`${resourceName} : ${collectedRewards[resourceName]}`)
    }
