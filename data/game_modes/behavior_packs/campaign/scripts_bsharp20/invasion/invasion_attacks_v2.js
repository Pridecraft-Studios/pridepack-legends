...
const _SetOvertimePiglins = (invasionV2Id, piglins) => {
    ...
    if (QUERY_GetEntitiesCount(piglins) < overtimeData.minimumCount) {
        ...
        OUTPUT_SetLeash(spawnedUnits, GetVillageEntityFromID(villageId), invasionAttackV2Data.common.leashDistance, invasionAttackV2Data.common.leashEndDistance)
...
const AttackV2LaunchNextWaveAttack = (invasionV2Id, overrideSpawnLocation = [], customMultiplier = -1, assignedControlGroup = undefined) => {
    ...
    OUTPUT_SetNamedTimer(invasionAttackV2Data.common.timers.attackV2NextWaveTimer, waveWaitTime)

    if (!GetVillageVariable(villageId, invasionAttackV2Data.common.vars.attackActive)) {
        OUTPUT_DebugLogInvasion(`Invasion Attack V2: New wave spawn missed due to attack being abandoned!`)
        SetVillageVariable(villageId, invasionAttackV2Data.common.vars.attackWavesMissed, 1)
        return
    }

    // Reset compass icons in preparation for spawning a new wave.
    ...
    if (_SpawnAndLaunchUnits(invasionV2Id, currentWave, false, overrideSpawnLocation, customMultiplier, assignedControlGroup)) {
        ...
        LISTENFOR_LocalTimer({
            snippet: "lt_play_new_wave_vo",
            ownerVillageId: villageId,
            waitTime: invasionAttackV2Data.common.newWaveVOWaitTime
        })
    }
}

SNIPPET_LocalTimer("lt_play_new_wave_vo", (payload) => {
    const villageId = payload.ownerVillageId
    if (GetVillageVariable(villageId, invasionAttackV2Data.common.vars.canNewWaveVOPlay) === 0) {
        const villageEntity = GetVillageEntityFromID(villageId)
        PlayPresentationActionToClosePlayers(invasionAttackV2Data.common.VO.newWave, villageEntity, invasionAttackV2Data.common.presentationActionDistance)
        SetVillageVariable(villageId, invasionAttackV2Data.common.vars.canNewWaveVOPlay, 1)

        LISTENFOR_LocalTimer({
            snippet: "lt_new_wave_vo_enable",
            ownerVillageId: villageId,
            waitTime: invasionAttackV2Data.common.newWaveVOCooldown
        })
    }
})

SNIPPET_LocalTimer("lt_new_wave_vo_enable", (payload) => {
    SetVillageVariable(payload.ownerVillageId, invasionAttackV2Data.common.vars.canNewWaveVOPlay, 0)
})

const _AttackV2LaunchCurrentWaveAttack = (invasionV2Id, overrideSpawnPoints, customMultiplier = -1, assignedControlGroup = undefined) => {
    const villageId = QUERY_GetInvasionAttackV2VillageId(invasionV2Id)
    const strength = QUERY_GetInvasionAttackV2Strength(invasionV2Id)
    const factionName = QUERY_GetInvasionAttackV2FactionName(invasionV2Id)
...
const _SpawnAndLaunchUnits = (invasionV2Id, currentWave, isFirst = false, overrideSpawnLocations = [], customMultiplier = -1, assignedControlGroup = undefined) => {
    ...
    for (let i = numberOfGroups - 1; i >= 0; --i) {
        ...
        if (currentWave.unitPreset !== undefined) {
            if (currentWave.unitPreset.preset === undefined) {
                Loge(`Invasion Attack V2: Failed to retrieve Preset wave data for invasion attack!`)
                return false
            }
        ...
        if (strength === 4 && customMultiplier === 1) {
            ...
            Logw("invalid strength for village attack. Setting the max piglins active to the highest strength level.")
        } else {
        ...
        if (numInvasionPiglins > maxPiglins) {
            ...
            Logi("piglins blocked from spawning due to unit cap for strength level")
            return false
        }
    }
    ...
    for (let i = numberOfGroups - 1; i >= 0; --i) {
        ...
        if (HasEntities(newUnits)) {
            ...
            OUTPUT_SetLeash(newUnits, GetVillageEntityFromID(villageId), invasionAttackV2Data.common.leashDistance, invasionAttackV2Data.common.leashEndDistance)
...
SNIPPET_InvasionAttackV2Activated("invasion_attackV2_activated", (attackV2Data, percentComplete, timePast) => {
    ...
    SetVillageVariable(targetVillageId, invasionAttackV2Data.common.vars.lateStartPercent, percentComplete * 100)
    ...
    if (GetVillageVariable(targetVillageId, invasionAttackV2Data.common.vars.gracePeriodActive)) {
        ...
        if (villageSize !== VILLAGE_SIZE_PLAYER) {
            ...
            if (IsThisAct1VillageAttack(targetVillageId)) {
                ...
                const villageFaction = QUERY_GetFactionNameFromVillageID(targetVillageId)
                const cinematicData = invasionAttackV2Data.common.cinematics[villageFaction].spawn
...
SNIPPET_InvasionAttackV2Reactivated("invasion_attackV2_reactivated", (attackV2Data) => {
    ...
    _CreateDamageRequest(invasionV2Id, villageId, damagePercent, false)
...
SNIPPET_InvasionAttackV2DamageRequest("invasion_attackV2_damage_request", (attackV2Data, payload) => {
    ...
    if (missedWaves > 0) {
        ...
        _AttackV2LaunchCurrentWaveAttack(invasionV2Id, overrideSpawnPoints, customMultiplier)
...
SNIPPET_InvasionAttackV2Resolve("invasion_attackV2_resolve", (attackV2Data) => {
    ...
    // If the attack has never activated, then we apply full damage.
    let percentMod = 1
    const currentCompletePercent = QUERY_GetInvasionAttackV2PercentComplete(attackV2Data.id)
    if (currentCompletePercent !== 0) {
        // Otherwise, scale the damage based on how much of the fight has been missed.
        percentMod = currentCompletePercent - _GetPercentCompleteSinceAbandon(villageId)
    }
...
SNIPPET_InvasionAttackV2Ended("invasion_attackV2_ended", (attackV2Data, _playerDefended) => {
    ...
    if (defenseSuccess && playerPresent) {
        ...
        if (isPlayerOutpost) {
            ...
            //Play a cinematic unless you just killed the boss
            if (!QUERY_GetGlobalVariable("hammerBossKilled_stopVillageCin")) {
                //Pick relevant cine data
                const cinematicData = invasionAttackV2Data.common.cinematics[villageFaction].saved
    ...
    } else if (!defenseSuccess && !isPlayerOutpost) {
...
SNIPPET_GlobalVariableChanged("gvc_attackV2_obs_generating", (_oldValue, newValue, payload) => {
    ...
    if (newValue <= 0) {
        ...
        if (initialWaveTimer === 0) {
            ...
            OUTPUT_SetNamedTimer(invasionAttackV2Data.common.timers.attackV2NextWaveTimer, initialWaveTimer)
        }
        ...
        LISTENFOR_Timer({
            ...
            payloadInt: invasionV2Id,
            ownerVillageId: targetVillageId,
            timerName: invasionAttackV2Data.common.timers.attackV2NextWaveTimer,
            threshold: 0
        })

        // Damage the village according to how far ahead this attack is if we started late.
        if (GetVillageVariable(targetVillageId, invasionAttackV2Data.common.vars.attackWavesMissed) > 0) {
            const lateStartPercent = GetVillageVariable(targetVillageId, invasionAttackV2Data.common.vars.lateStartPercent) / 100
            _CreateDamageRequest(invasionV2Id, targetVillageId, lateStartPercent, false)
        }
