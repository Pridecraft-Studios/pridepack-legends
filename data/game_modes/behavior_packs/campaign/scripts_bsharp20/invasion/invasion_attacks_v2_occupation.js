...
const occupationValPiglins = {
    ...
    [FACTION_NAME_DEFEND]: {
        ...
        occupyingUnits: [
            ...
            { unit: PIGLIN_ARCHETYPE.PORTAL_GUARD, count: 2, reinforce: 0, diminish: 1, unitTag: "portalguard" }
        ]
    },
    [FACTION_NAME_OBSTACLE]: {
        ...
        occupyingUnits: [
            ...
            { unit: PIGLIN_ARCHETYPE.LAVA_LAUNCHER, count: 2, reinforce: 0, diminish: 1, unitTag: "piggo_lava_launcher" }
        ]
    },
    [FACTION_NAME_DBB]: {
        abandonedTickTime: 60,
...
SNIPPET_EntitySpawned("es_attack_horde_spawned_occupation", (bossEntity) => {
    ...
    OUTPUT_SetLeash(bossEntity, GetVillageEntityFromID(villageId), occupationValCommon.leashDistance, occupationValCommon.leashEndDistance)
...
const _SpawnPiglinsForOccupation = (villageId, hordeName) => {
    ...
    for (const config of hordeOccupationData.occupyingUnits) {
        ...
        const newPiglins = SpawnEntitiesInVillage(config.unit, reinforcementCount, TEAM_ORANGE, villageId, 20, 50, hordeName)
    ...
    if (HasEntities(piglins)) {
        ...
        OUTPUT_SetLeash(piglins, GetVillageEntityFromID(villageId), occupationValCommon.leashDistance, occupationValCommon.leashEndDistance)
    }
...
const _InitReinforcementPools = (villageId, hordeName, attackStrength) => {
    // Setup initial reinforcements for occupation if no players are present.
